...
var stdErr = function (p, n) {
    return n != 0 ? Math.sqrt(p * (1 - p) / n) : 0.0;
};
...
var conversionRate = function (conversions, views) {
    return views != 0 ? conversions / views : 0.0;
}
...
var zScore = function (conversionsA, viewsA, conversionsB, viewsB) {
    ...
    var seNorm = Math.sqrt(seA * seA + seB * seB);
    return seNorm != 0 ? (pB - pA) / seNorm : 0.0;
}
